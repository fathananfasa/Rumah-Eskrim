<x-app-layout>
    <div class="py-6 bg-[#fff8e6] min-h-screen">
        <div class="px-4 sm:px-6">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
                ...
                <div>
                    <button id="btnTambah"
                        class="bg-purple-200 hover:bg-purple-300 text-purple-900 px-3 sm:px-4 py-2 rounded-lg flex items-center gap-2 text-sm sm:text-base">
           ...
           @if (session('error'))
        <div class="max-w-6xl mx-auto mt-4 bg-red-100 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
            {{ session('error') }}
        </div>
        @endif
        ...
        {{-- PESAN SUKSES --}}
        @if (session('success'))
        <div class="max-w-6xl mx-auto mt-4 bg-green-100 border border-green-200 text-green-700 px-4 py-3 rounded-lg">
            {{ session('success') }}
        </div>
        @endif
                    ...
                    <tbody>
                        ...
                        @endif">
                            ...
                            <td class="px-4 py-2 space-y-2 sm:space-y-0 sm:space-x-2 flex flex-col sm:flex-row">
                                ...
                                <form action="{{ route('produk.destroy', $item->id_prod) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-red-100 text-red-700 px-3 py-1 rounded w-full sm:w-auto">
                                        Hapus
                                    </button>
                                </form>
                            </td>
                        ...
                        <tr class="border-b">
                            ...
                            <td class="px-4 py-2 space-y-2 sm:space-y-0 sm:space-x-2 flex flex-col sm:flex-row">
                                ...
                                <form action="{{ route('perlengkapan.destroy', $item->id_per) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-red-100 text-red-700 px-3 py-1 rounded w-full sm:w-auto">
            ...
            <div class="sm:hidden space-y-4 mt-4">
                ...
                if ($persen <= 25) {
                    ...
                    <div class="flex gap-2 mt-3">
                        ...
                        <form action="{{ route('produk.destroy', $item->id_prod) }}" method="POST" class="flex-1">
                            @csrf
                            @method('DELETE')
                            <button
                                type="submit"
                                class="bg-red-100 text-red-700 rounded px-3 py-1 text-xs w-full">
                                Hapus
                            </button>
                        </form>
                    </div>
            ...
            if ($item->stok <= 15) {
                ...
                $status='Menipis' ;
                    ...
                    <div class="flex gap-2 mt-3">
                        ...
                        <form action="{{ route('perlengkapan.destroy', $item->id_per) }}" method="POST" class="flex-1">
                            @csrf
                            @method('DELETE')
                            <button
                                type="submit"
    ...
    <!-- Modal Tambah Barang -->
    <div id="modalTambah"
        class="hidden fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center p-4 sm:p-0 z-50">
        ...
        <div class="bg-white rounded-2xl p-6 w-full max-w-md shadow-xl">
            ...
            <form id="formTambah" method="POST" action="{{ route('produk.store') }}">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700">Kategori</label>
                    <select id="tambahKategori" name="kategori" class="w-full border rounded px-3 py-2" required>
                        <option value="Es Krim" selected>🍦 Es Krim</option>
                        <option value="Perlengkapan">🧰 Perlengkapan</option>
                    </select>
                </div>
                ...
                <!-- STOK / JUMLAH -->
                <div class="mb-4">
                    <label class="block text-gray-700" id="tambahLabel">Level Stok (%)</label>

                    <!-- Dropdown persentase untuk Es Krim -->
                    <select id="tambahPersentase"
                        name="stok"
                        class="w-full border rounded px-3 py-2">
                        <option value="100">🟢 100% (Full)</option>
                        <option value="75">🟡 75% (¾)</option>
                    ...
                    <!-- Input jumlah pcs untuk Perlengkapan -->
                    <input type="number" id="tambahJumlah"
                        name="stok"
                        class="hidden w-full border rounded px-3 py-2 mt-2"
    ...
    <script>
        ...
        const kategoriSelect = document.getElementById('tambahKategori');
        const label = document.getElementById('tambahLabel');
        const persentase = document.getElementById('tambahPersentase');
        const jumlah = document.getElementById('tambahJumlah');
        const nama = document.getElementById('tambahNama');
        ...
        kategoriSelect.addEventListener('change', e => {
            ...
            if (kategori === "Perlengkapan") {
                ...
                persentase.classList.add('hidden');
                persentase.required = false;
                persentase.removeAttribute('name');

                jumlah.classList.remove('hidden');
                jumlah.required = true;
                jumlah.setAttribute('name', 'stok');

                form.action = "{{ route('perlengkapan.store') }}";
                nama.setAttribute('name', 'nama_perlengkapan');

            } else { // Es Krim
                label.textContent = "Level Stok (%)";
                persentase.classList.remove('hidden');
                persentase.required = true;
                persentase.setAttribute('name', 'stok');

                jumlah.classList.add('hidden');
                jumlah.required = false;
                jumlah.removeAttribute('name');

                form.action = "{{ route('produk.store') }}";
                nama.setAttribute('name', 'nama_produk');
            }
        });

        // Pastikan value dikonversi saat submit
        form.addEventListener('submit', e => {
            if (kategoriSelect.value === 'Es Krim') {
                persentase.value = parseInt(persentase.value);
            } else {
        ...
        document.querySelectorAll('.editBtn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                form.action = (kategori === "Es Krim") ?
                    "{{ url('produk') }}/" + btn.dataset.id :
                    "{{ url('perlengkapan') }}/" + btn.dataset.id;
            });
        });
